...
def std_los_velocity_vs_rmu(
  data_filename1, output_filename,
  box_size, dim1_min, dim1_max,
  dim1_nbin, ngrid, data_filename2=None, 
  nthreads=1, dim2_min=-1, dim2_max=1,
  dim2_nbin=80
):
  '''
  Velocity dispersion profile as a function of radial
  and LOS angle separation.
  ...
  dim1_min: minimum pair distance.

  dim1_max: maximum pair distance.

  dim1_nbin: number of radial bins.
  ...
  dim2_min: (optional) minimum mu separation.

  dim2_max: (optional) maximum mu separation .

  dim2_nbin: (optional) number of mu bins.
  '''
  ...
  binpath = path.join(path.dirname(__file__),
  'bin', 'std_los_velocity_vs_rmu.exe')
